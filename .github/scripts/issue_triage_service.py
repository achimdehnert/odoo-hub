"""
IssueTriage Service — ADR-085 (odoo-hub, standalone).
"""
...
GITHUB_REPO = os.environ.get("GITHUB_REPOSITORY", "achimdehnert/odoo-hub")
...
PATH_APP_LABELS: list[tuple[str, str]] = [('addons/', 'app:addons'), ('tests/', 'scope:tests'), ('.github/', 'scope:ci'), ('docker/', 'scope:infrastructure')]
...
class IssueTriageService:
    ...
    def triage(self, issue_number: int, title: str, body: str = "", existing_labels=None) -> TriageResult:
        ...
        use_case = f"{title}\n\n{body}".strip()
        context = 'Repo: odoo-hub, Stack: Python/Odoo, Odoo customizations and addons'
